...
class KoleksiBuku {
    ...
    public perbaruiInfoBuku(id: number, judul: string, penulis: string, tahunTerbit: number, detailSpesifik: string): void {
        const buku = this.cariBukuBerdasarkanId(id);
        if (buku) {
            // Memperbarui properti umum
            buku.judul = judul;
            buku.penulis = penulis;
            buku.tahunTerbit = tahunTerbit;
            // Memperbarui properti spesifik berdasarkan jenis buku
            if (buku instanceof BukuFiksi) buku.subGenre = detailSpesifik;
            else if (buku instanceof BukuNonFiksi) buku.topik = detailSpesifik;
            this.perbaruiTampilanDaftarBuku();
        }
    }
        ...
        if (outputElement) {
            ...
            outputElement.innerHTML = this.daftarBuku.length === 0 ? "<p>Belum ada buku dalam koleksi.</p>" :
                "<h2>Koleksi Buku:</h2>" + this.daftarBuku.map(buku => `
                    <div>
                        ${buku.id}. ${buku.getDeskripsiLengkap()}
    ...
    public hapusBukuDariKoleksi(id: number): void {
        // Filter out buku dengan id yang sesuai
    ...
    public prosesFormBuku(): void {
        ...
        if (this.validasiInputBuku(judul, penulis, tahunTerbit, jenisBuku, detailSpesifik)) {
            if (this.idBukuYangSedangDiedit !== null) {
                ...
                this.koleksiBuku.perbaruiInfoBuku(this.idBukuYangSedangDiedit, judul, penulis, tahunTerbit, detailSpesifik);
